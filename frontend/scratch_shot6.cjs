...
(async () => {
  ...
  await page.screenshot({ path: `${out}/topbar1.png` });

  // click the first room chip to open its dropdown
  const chip = await page.$('.room-chip-summary');
  await chip.click();
  await page.waitForTimeout(300);
  await page.screenshot({ path: `${out}/topbar2_chip_open.png` });

  // close it, then open the notification bell
  await chip.click();
  await page.waitForTimeout(200);
  const bell = await page.$('.notif-bell');
  await bell.click();
  await page.waitForTimeout(300);
  await page.screenshot({ path: `${out}/topbar3_bell_open.png` });
